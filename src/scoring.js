const WebSecScoring = (() => {
  ...
  function parsePolicy(policy) {
    ...
    for (const part of policy.split(";")) {
      ...
      if (name && !(name.toLowerCase() in result)) result[name.toLowerCase()] = tokens;
    }
    return result;
  }
  ...
  function analyze(data) {
    ...
    const availability = data.availability || {headers:true, page:true, cookies:true};
    if (availability.headers) {
    ...
    if (!csp) add("warn", "hardening", "Content-Security-Policy", "No se detectó una política CSP.", "Define una CSP adaptada a los recursos legítimos del sitio.", 10);
      ...
      const directives = parsePolicy(csp);
      const scripts = directives["script-src-elem"] || directives["script-src"] || directives["default-src"] || [];
      const execution = directives["script-src"] || directives["default-src"] || [];
      const weak = [];
      if (execution.includes("'unsafe-eval'")) weak.push("unsafe-eval en scripts");
      const nonceOrHash = scripts.some(t => /^'(nonce-|sha(256|384|512)-)/.test(t));
      if (scripts.includes("'unsafe-inline'") && !nonceOrHash) weak.push("scripts inline sin nonce/hash");
      if (scripts.some(t => t === "*" || /^(https?|data):$/.test(t))) weak.push("fuentes de scripts amplias");
      if (!directives["script-src"] && !directives["script-src-elem"] && !directives["default-src"]) weak.push("scripts sin restricción");
      if (!directives["object-src"] && !directives["default-src"]) weak.push("objetos sin restricción");
    ...
    const ancestors = parsePolicy(csp)["frame-ancestors"];
    const frameAncestors = !!ancestors?.length && !ancestors.some(t => t === "*" || /^(https?|data):$/.test(t)) && !(ancestors.includes("'none'") && ancestors.length > 1);
    if (frameAncestors || (!ancestors && /^(deny|sameorigin)$/i.test(frame))) add("pass", "hardening", "Protección contra marcos", "Se detectó una política contra inclusión no autorizada en marcos.", "Mantén frame-ancestors o X-Frame-Options según compatibilidad.", 0, frameAncestors ? "CSP frame-ancestors" : frame);
    ...
    } else add("unknown", "coverage", "Cabeceras", "No se capturaron las cabeceras de esta navegación.", "Recarga la página y vuelve a analizar; no se penaliza lo desconocido.");
    ...
    if (!cookies.length) add("info", "info", "Cookies", "No se detectaron cookies accesibles para esta URL.", "No requiere acción.");
      ...
      if (sessionReadable.length) add("warn", "hardening", "Cookies HttpOnly", `${sessionReadable.length} cookies temporales son accesibles desde JavaScript; no implica que sean de autenticación.`, "Usa HttpOnly en cookies de sesión que no necesiten acceso desde scripts.", 0);
    ...
    } else add("unknown", "coverage", "Cookies", "No se pudieron consultar las cookies.", "Comprueba el acceso de la extensión a este sitio.");
    if (availability.page) {
    ...
    } else add("unknown", "coverage", "Página y formularios", "No se pudo inspeccionar el contenido.", "Recarga la página y comprueba los permisos.");
    const redirects = data.redirects || 0;
    if (availability.headers) add(redirects >= 4 ? "warn" : "info", "info", "Redirecciones", `${redirects} redirecciones detectadas.`, redirects >= 4 ? "Revisa la cadena para reducir complejidad y exposición." : "No requiere acción.", redirects >= 4 ? 2 : 0);
    const third = data.thirdPartyOrigins?.length || 0;
    if (availability.page) add(third > 20 ? "warn" : "info", "info", "Terceros", `${third} orígenes externos observados.`, third > 20 ? "Revisa si todos los terceros son necesarios y confiables." : "Evalúa cada tercero según su finalidad.", third > 20 ? 2 : 0);

    const score = Math.max(0, 100 - findings.reduce((sum, item) => sum + item.penalty, 0));
    const counts = findings.reduce((out, item) => (out[item.status]++, out), { pass: 0, warn: 0, fail: 0, info: 0, unknown: 0 });
    return { score, findings, counts, coverage: {checked: Object.values(availability).filter(Boolean).length, total:3, complete: Object.values(availability).every(Boolean)}, totalPenalty: 100 - score };
  }
  return { analyze };
})();
